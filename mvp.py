...
class mvp(object):
    ...
    def __init__(self, index0, value0):
        ...
        '''
        index0,value0 = np.array(index0,dtype=int),np.array(value0,dtype=complex)
    ...
    def __repr__(self):
        ...
        '''
        s = '\n'
        formt = self.index.shape[1]*'%2d ' + '%6.6g    +%6.6gi \n'
        for i in range(self.index.shape[0]):
            temp = np.append(self.index[i,:], [self.value[i].real,self.value[i].imag])
            s += formt % tuple(temp)
        s += '\n'
        #s += '----------\n# of terms: %d\n'%(self.index.shape[0])
        return s
    ...
    def simplify(self):
        ...
        '''
        if self.index.shape[0]==0: return
        # combine like terms
    ...
    def __mul__(self, b):
        ...
        D = np.zeros(na*nb,dtype=complex)
        ci = 0
        for i in xrange(na):
            for j in xrange(nb):
