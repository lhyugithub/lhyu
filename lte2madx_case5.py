...
def line(le):
    ele = le.split("=")[1]
    le = (
        le.split("=")[0] + "=" + ele.replace("(", "(").replace("&", "")
    )  # .replace('MA1,','')
    return le[:-1] + ";\n"


def CSBEND(le):
    ele = le.split(":")[0] + le.split(":")[1].replace(" CSBEND,", ":SBEND, ").replace(
        ",  N_KICKS= 40", ""
    ).replace("\n", ";\n")
    return ele
...
def drif(le):
    ele = le.split(":")[0] + le.split(":")[1].replace(" EDRIFT,", ":DRIFT,").replace(
        " drif,", ":DRIFT,"
    ).replace(" DRIF,", ":DRIFT,").replace("\n", ";\n")
    return ele
...
def kquad(le):
    ele = le.split(":")[0] + le.split(":")[1].replace(
        " KQUAD,", ":QUADRUPOLE,"
    ).replace(" kquad,", ":QUADRUPOLE,").replace(",  N_KICKS= 40", "").replace(
        "\n", ";\n"
    )
    return ele
...
def kSext(le):
    ele = le.split(":")[0] + le.split(":")[1].replace(" KSEXT,", ":SEXTUPOLE,").replace(
        " kSext,", ":SEXTUPOLE,"
    ).replace(",  N_KICKS= 40", "").replace("\n", ";\n")
    return ele
...
for ik in [1]:
    ...
    separator = ","
    a2 = [",".join([",".join(a1[i]) for i in j]).replace("&\n,", "\n") for j in lamper3]
    # inser lines without '&\n' back into list
    a3 = []
    k = 0
    for i in range(len(lamper3)):
        for n in range(k, lamper3[i][0]):
            a3.append(separator.join(a1[n]))
        a3.append(a2[i])
        k = lamper3[i][-1] + 1
    a4 = a3 + a0[lamper3[-1][-1] + 1 : -1]
    spt = ""
    ...
    for i, le in enumerate(a):
        ...
        if len(ele) > 1 and len(ele[1].split(",")[0].split()) > 0:
            ...
            if ele == "csbend":
                ...
                t = CSBEND(le)
            elif ele == "drif" or ele == "EDRIFT" or ele == "DRIF":
                t = drif(le)
            elif ele == "kick":
                t = kick(le)
            elif ele == "HKICK" or ele == "MALIGN" or ele == "MARK" or ele == "VKICK":
                t = HKICK(le)
            elif ele == "MULT":
                t = HKICK(le)
            elif ele == "watch" or ele == "MONI" or ele == "moni":
                t = moni(le)
            elif ele == "WATCH":
                t = moni(le)
...
os.system("cat  madx_case5_tail1 >>" + " " + fn + ".madx")
